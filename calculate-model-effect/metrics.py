...
def LAT_RW(start_time, end_time):
	...
	cursor.execute("SELECT month(author_date), delay FROM "
		...
		"where author_date between %s and %s "
		"and scmlog.hash = hash_file.hash "
		"and hash_file.file_name like 'drivers/%%') as tmp ",
		(start_time, end_time))
	res = cursor.fetchall()
	for i in range(len(res)):
		if res[i][0] in month.keys():
			month[res[i][0]].append(res[i][1])
		else:
			month[res[i][0]]=[res[i][1]]
...
if __name__ == '__main__':
	conn = pymysql.connect(host='127.0.0.1', port=3306, user='root', passwd='*****', db='maintainer', charset='utf8')
	cursor = conn.cursor()
	...
	for i in range(0,11):
		...
		m1=self.MAX_RW(start_time, end_time)
		m2=self.INTST_RW(start_time, end_time)
